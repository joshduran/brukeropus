...
if __name__ == "__main__":
    file_dirs = [
        r'C:\Users\duranjm\Desktop\examples\files',
        r'C:\Users\duranjm\Desktop\examples\files-3d',
        r'D:\Lab Files\PL Data',
        r'D:\Lab Files\FTIR',
    ]
    ...
    #directory = file_dirs[3]  # Directory with test files
    width = 120  # Width of report in characters
    print_non_opus_ext = True  # Whether to print all files without OPUS numeric extension
    #-----------------------------------------------------------------------------------------------
    # Initialize Data
    opus_files = find_opus_files(directory, recursive=True)
    opus_data = [read_opus(f) for f in opus_files]
    for o in opus_data:
        o.rel_path = '\\' + os.path.relpath(o.filepath, directory)  # Add rel_path attribute for convenience
    #-----------------------------------------------------------------------------------------------
    # Print header
    print()
    print('=' * width)
    ...
    print('.' * width)
    if len(redundant) == 0:
        print_pass('OPUSFile directory blocks are clear (No redundant data blocks in class)')
    else:
    ...
    print('.' * width)
    if len(bad_parse) == 0:
        print_pass('Every block was sucessfully parsed')
    else:
