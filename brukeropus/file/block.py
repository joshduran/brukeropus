...
class FileBlock:
    ...
    as well as the raw bytes from the file (to assign block bytes).
    ...
    def is_param(self):
        '''Returns True if `FileBlock` is a parameter block (could be data status, rf param, sample param, etc.)'''
        return self.type[2] > 0 or self.type == (0, 0, 0, 0, 0, 1)

    def is_sm_param(self):
        '''Returns True if `FileBlock` is a prameter block associated with sample/result measurement (not including
        data status blocks)'''
...
class FileDirectory:
    ...
    supports. Then it decodes the block pointer information from each entry of the file's directory block.  This class
    intially holds all the parsed blocks from the file in `self.blocks`.  As these blocks are assigned to `OPUSFile`
    attributes, they are removed from `self.blocks` to save memory.
...
def is_data_status_type_match(data_block: FileBlock, data_status_block: FileBlock) -> bool:
    ...
    (e.g. when multiple spectra of the same type are stored in the file)'''
...
def is_data_status_val_match(data_block: FileBlock, data_status_block: FileBlock) -> bool:
    '''Checks if min(data) and max(data) match up with the data status: MNY and MXY.
    ...
    if data_block.is_data():
        ...
        return True # Don't rule out data series


def is_valid_match(data_block: FileBlock, data_status_block: FileBlock) -> bool:
    '''Checks that number of points in data status are less than or equal to length of parsed data.
...
def pair_data_and_status_blocks(blocks: list) -> list:
    ...
    inspection to accurately pair.  This function uses the following logical sequence to accurately pair these blocks:
        ...
        1. Pair by type and isolate singular matches
        2. For duplicate matches, check min(data) and max(data) match data status MNY and MXY (including CSF scaling)
        3. For matches that are still duplicate, remove any matches that are already in the singular match list.
        4. Remove and matches from the singular match list where len(data) is < data status NPT (invalid data block)
        5. Sorts the pairs in reverse order of where the data blocks are stored in the file (presume last is most
    ...
    plausible that it will not be sufficient for all files that have duplicate data type entries.  
    '''
