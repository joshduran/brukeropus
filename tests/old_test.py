...
def filter_data_with_mismatched_num_blocks(data: list) -> list:
    ...
    for o in data:
        ...
        if not (
            ...
            and len(blocks) == o.directory.num_blocks
        ):
            mismatched.append(o)
    return mismatched
...
def filter_data_with_unknown_blocks(data: list) -> list:
    ...
    return [
        d
        for d in unknown
        if any(b.type != (0, 0, 0, 0, 0, 0) for b in d.unknown_blocks)
    ]
...
if __name__ == "__main__":
    ...
    print_non_opus_ext = (
        False  # Whether to print all files without OPUS numeric extension
    )
    # ----------------------------------------------------------------------------------------------
    # Initialize Data
    ...
    for o in opus_data:
        o.rel_path = "\\" + os.path.relpath(
            o.filepath, directory
        )  # Add rel_path attribute for printing
    # ----------------------------------------------------------------------------------------------
    # Print header
    print()
    print("=" * width)
    ...
    if len(mismatched) == 0:
        print_pass(
            "All file blocks were found and consistent ("
            + str(len(opus_data))
            + " files)"
        )
    else:
        print_fail(
            "Some mismatched block numbers: "
            + str(len(mismatched))
            + "/"
            + str(len(opus_data))
            + " files"
        ...
        for o in mismatched:
            print(
                ...
                o.rel_path,
                "\n    num_blocks:",
                o.directory.num_blocks,
                "\n    directory block length:",
                len(o.directory.block.data),
                "\n    length of blocks found in OPUSFile:",
                len(get_all_blocks(o)),
            )
    # ----------------------------------------------------------------------------------------------
    ...
    if len(redundant) == 0:
        print_pass(
            "OPUSFile.directory.blocks are all clear (No redundant data blocks in class)"
        )
    else:
        print_fail(
            "Redundant data blocks found in OPUSFile.directory.blocks (Memory ineffiency):"
        )
        for o in redundant:
            print("   ", o.rel_path)
    # ----------------------------------------------------------------------------------------------
    ...
    if len(unknown) == 0:
        print_pass(
            "No files with unknown/uncategorized blocks [Ignoring (0, 0, 0, 0, 0, 0) blocks]"
        )
    else:
        print_fail(
            "Files with unknown blocks: "
            + str(len(unknown))
            + "/"
            + str(len(opus_data))
            + " files"
